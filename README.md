...
Download **v0.1.1** from [GitHub Releases](https://github.com/aminezombor/SimForge/releases/tag/v0.1.1):

- `SimForge-Setup.exe` — Windows installer (unsigned)
- `SimForge-win32-x64-0.1.1.zip` — portable SmartScreen fallback
- `simforge_bridge-0.1.0.zip` — Blender extension
- `SimForge-Warehouse-Sample-0.1.1.zip` — sanitized sample project and verified USD
...
5. Use **Guided** authority, open Build, review the warehouse plan, approve the
   checkpointed build, run validation, inspect the deliberate gripper finding, approve
   correction, then explicitly export the canonical USD package.
6. Confirm the export reports `verified`, opens after relocation, and includes
   `manifest.json`, validation JSON, and the readiness report.
...
Codex with GPT-5.6 translated the master brief into 125 traceable requirements, evaluated
